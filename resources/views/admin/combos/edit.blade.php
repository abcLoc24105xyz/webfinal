{{-- resources/views/admin/combos/edit.blade.php (Đã đồng bộ) --}}
@extends('admin.layouts.app')

@section('title', 'Sửa Combo #'.$combo->combo_id)
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6 sm:p-8">
    ...
    <div class="max-w-4xl mx-auto bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 ring-1 ring-white/5">
        ...
        {{-- Header --}}
        <h1 class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 mb-8 flex items-center gap-3">
            <i class="fas fa-edit"></i> SỬA COMBO: {{ $combo->combo_name }}
        </h1>
        ...
        <form action="{{ route('admin.combos.update', $combo) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="filter-box">
                    ...
                    <input type="text" name="combo_name" value="{{ old('combo_name', $combo->combo_name) }}"
                            class="modern-input placeholder-white/50" required placeholder="Ví dụ: Combo Bắp + Nước">
                ...
                <div class="filter-box">
                    ...
                    <input type="number" name="price" value="{{ old('price', $combo->price) }}"
                            class="modern-input placeholder-white/50" required placeholder="Ví dụ: 99000">
            ...
            <div class="mt-6 filter-box">
                ...
                <textarea name="description" rows="4" placeholder="Mô tả chi tiết combo..."
                            class="modern-input placeholder-white/50">{{ old('description', $combo->description) }}</textarea>
                @error('description')<p class="text-pink-400 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            {{-- 4. Hình ảnh --}}
            <div class="mt-8 border-t border-white/10 pt-8">
                <label class="block text-purple-300 font-bold mb-4 text-xl"><i class="fas fa-image mr-2"></i> Quản lý Hình ảnh</label>
                
                {{-- Hiển thị ảnh hiện tại --}}
                <div class="mb-6 flex justify-center">
                    <div class="relative w-72 h-48 rounded-xl overflow-hidden shadow-2xl border-4 border-white/20 p-1 bg-black/50">
                        @if($combo->image)
                        ...
                        @else
                            <div class="w-full h-full bg-gray-700 border-4 border-dashed border-purple-400 rounded-lg flex flex-col items-center justify-center text-purple-300">
                                <i class="fas fa-times-circle text-4xl mb-2"></i>
                                <span class="font-bold">Chưa có ảnh</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="filter-box">
                    <label class="filter-label">
                        <i class="fas fa-upload"></i> Thay ảnh mới (JPG, PNG, WEBP - Max 2MB)
                    </label>
            ...
            <div class="mt-8 border-t border-white/10 pt-8">
                ...
                <div class="flex items-center space-x-10 bg-black/50 p-4 rounded-xl border border-white/10">
                    ...
                    <label class="flex items-center cursor-pointer group">
                        <input type="radio" name="status" value="1" {{ old('status', $combo->status) == 1 ? 'checked' : '' }}
                                class="w-5 h-5 text-green-500 border-green-500 focus:ring-green-500 bg-gray-800 checked:bg-green-600">
                        <span class="ml-3 text-lg text-green-400 font-bold group-hover:text-green-300 transition">1 - Đang hoạt động</span>
                    </label>
                    
                    {{-- Đã ẩn --}}
                    <label class="flex items-center cursor-pointer group">
                        <input type="radio" name="status" value="0" {{ old('status', $combo->status) == 0 ? 'checked' : '' }}
                                class="w-5 h-5 text-gray-400 border-gray-400 focus:ring-gray-400 bg-gray-800 checked:bg-gray-600">
                        <span class="ml-3 text-lg text-gray-400 group-hover:text-gray-300 transition">0 - Đã ẩn</span>
                    </label>
                </div>
            ...
            <div class="mt-10 flex justify-end space-x-4 border-t border-white/10 pt-6">
                ...
                <button type="submit"
                        class="px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-black rounded-xl shadow-2xl transition transform hover:scale-105 flex items-center gap-2">
...
{{-- Bổ sung lại các style CSS đã định nghĩa từ trang index --}}
<style>
    ...
    .modern-input {
        ...
        border: none;
        padding: 0;
        line-height: 1.5; /* Giúp textarea trông đẹp hơn */
    }
</style>
@endsection